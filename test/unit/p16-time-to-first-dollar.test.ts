...
import { firstRevenueAt } from '../../lib/db/roi.ts';
...
describe('firstRevenueAt', () => {
  test('returns null when no revenue records exist', () => {
    ...
    db.prepare('INSERT INTO projects (id, name, type, status, created_at) VALUES (?, ?, ?, ?, datetime(\'now\'))').run(projectId, 'Test Project', 'code_repo', 'active');
  ...
  test('returns null when only non-revenue records exist', () => {
    ...
    db.prepare('INSERT INTO roi_records (project_id, kind, amount, recorded_at) VALUES (?, ?, ?, ?)').run(projectId, 'cost', 500, '2024-01-02T00:00:00Z');
  ...
  test('handles multiple projects correctly', () => {
    ...
    db.prepare('INSERT INTO projects (id, name, type, status, created_at) VALUES (?, ?, ?, ?, datetime(\'now\'))').run(projectId1, 'Project 1', 'code_repo', 'active');
    db.prepare('INSERT INTO projects (id, name, type, status, created_at) VALUES (?, ?, ?, ?, datetime(\'now\'))').run(projectId2, 'Project 2', 'code_repo', 'active');
    ...
    db.prepare('INSERT INTO roi_records (project_id, kind, amount, recorded_at) VALUES (?, ?, ?, ?)').run(projectId2, 'cost', 50, '2024-05-02T00:00:00Z');

    const result1 = firstRevenueAt(db, projectId1);
    const result2 = firstRevenueAt(db, projectId2);

    assert.equal(result1, '2024-05-01T00:00:00Z');
